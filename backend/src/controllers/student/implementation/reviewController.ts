...
export default class ReviewController {
  ...
  create = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
      ...
      if (!(r >= 1 && r <= 5)) {
        res.status(HttpStatus.BAD_REQUEST).json({ message: 'rating must be between 1 and 5' });
        return;
      }
  ...
  getById = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (!review) {
        res.status(HttpStatus.NOT_FOUND).json({ message: 'Review not found' });
        return;
      }
      res.json(review);
      return;
  ...
  update = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (!Object.keys(updates).length) {
        res.status(HttpStatus.BAD_REQUEST).json({ message: 'Nothing to update' });
      }
