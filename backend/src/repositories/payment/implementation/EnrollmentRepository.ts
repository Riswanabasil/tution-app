  ...
  implements IEnrollmentRepository
{
  ...
  async updateStatus(orderId: string, status: IEnrollment['status']): Promise<IEnrollment | null> {
  ...
  async findPaymentHistory(userId: string): Promise<(IEnrollment & { course: any })[]> {
    return EnrollmentModel.find({ userId: userId })
      .sort({ createdAt: -1 })
      .populate<{ course: any }>('courseId', 'title thumbnail price')
      .exec();
  }
  ...
  async countPaidByUser(userId: string): Promise<number> {
    return EnrollmentModel.countDocuments({ userId, status: 'paid' }).exec();
  }
