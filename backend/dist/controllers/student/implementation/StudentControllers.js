    ...
    registerStudent(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const { student, token } = yield this.studentService.registerStudentService(name, email, phone, password);
    ...
    loginStudent(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const { accessToken, refreshToken, student } = yield this.studentService.loginStudentService(email, password);
                ...
                res.status(statusCode_1.HttpStatus.UNAUTHORIZED).json({ message: errorMessages_1.ERROR_MESSAGES.UNAUTHORIZED });
            }
        });
    }
    ...
    refreshAccessToken(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                if (!refreshToken) {
                    res.status(statusCode_1.HttpStatus.UNAUTHORIZED).json({ message: 'No refresh token provided' });
                    return;
                }
                ...
                res.status(statusCode_1.HttpStatus.FORBIDDEN).json({ message: errorMessages_1.ERROR_MESSAGES.FORBIDDEN });
            }
        });
    }
    googleLoginStudent(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                if (!idToken) {
                    res.status(statusCode_1.HttpStatus.BAD_REQUEST).json({ message: 'Google ID token missing' });
                    return;
                }
                const REFRESH_COOKIE = Number(process.env.MAX_AGE);
                const { accessToken, refreshToken, student } = yield this.studentService.googleLoginStudentService(idToken);
                res.cookie('refreshToken', refreshToken, {
                    httpOnly: true,
                    secure: process.env.NODE_ENV === 'production',
                    sameSite: 'strict',
    ...
    getUploadUrl(req, res, next) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const data = yield (0, s3Presign_1.presignPutObject)({ keyPrefix: 'studentPic', filename, contentType });
                res.json(data);
            }
            catch (err) {
                next(err);
