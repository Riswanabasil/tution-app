...
//                   n <= rating ? 'border-yellow-400 bg-yellow-400/90' : 'border-gray-300 bg-gray-100'
//                 }`}
//                 aria-label={`Rate ${n}`}
//               >
//                 {n}
  ...
  return (
    ...
    >
      <div className="w-full max-w-md rounded-2xl bg-white p-4 sm:p-6 shadow-xl">
        <div className="mb-4">
          <h3 id="review-modal-title" className="text-lg sm:text-xl font-semibold text-gray-800">
            {initial ? 'Edit your review' : 'Rate & Review'}
          </h3>
          <p className="text-sm text-gray-500">Course ID: {courseId}</p>
        </div>
            ...
            {[1, 2, 3, 4, 5].map((n) => (
              <button
                ...
                className={`h-10 w-10 rounded-full border text-sm font-semibold transition-colors ${
                  n <= rating
                    ? 'border-yellow-400 bg-yellow-400/90 text-yellow-900'
        ...
        {/* Comment */}
        <div className="mb-4">
          <label className="mb-2 block text-sm font-medium text-gray-700" htmlFor="review-comment">
            Comment (optional)
          </label>
          <textarea
            id="review-comment"
            ...
            onChange={(e) => setComment(e.target.value)}
            rows={4}
            className="w-full rounded-lg border border-gray-300 p-3 outline-none focus:border-blue-500 focus:ring-2 focus:ring-indigo-100"
            placeholder="What did you like or dislike?"
          />
        </div>
        ...
        <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
          ...
          <button
            type="button"
            onClick={handleSubmit}
            className="w-full rounded-lg bg-gradient-to-r from-blue-600 to-purple-600 px-5 py-2 font-semibold text-white hover:from-blue-700 hover:to-purple-700 disabled:opacity-60 focus:outline-none focus:ring-2 focus:ring-indigo-300 sm:w-auto"
            disabled={loading}
          >
            {loading ? 'Saving...' : initial ? 'Update Review' : 'Submit Review'}
          </button>
