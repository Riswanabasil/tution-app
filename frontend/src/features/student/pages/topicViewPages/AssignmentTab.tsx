...
export default function AssignmentTab() {
  ...
  const getActionButton = (assignment: Assignment) => {
    // kept same actions but made buttons accessible and responsive
    switch (assignment.status) {
      case 'not submitted':
        return (
          <button
            type="button"
            onClick={() => handleOpenModal(assignment)}
            className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 px-4 py-2 text-sm font-medium text-white shadow-lg transition-transform duration-200 hover:scale-105 hover:from-blue-700 hover:to-indigo-700 hover:shadow-xl focus:outline-none focus:ring-2 focus:ring-indigo-300"
            aria-label={`Submit response for ${assignment.title}`}
          >
            <svg
              className="h-4 w-4"
              fill="none"
      ...
      case 'pending':
        return (
          <button
            type="button"
            onClick={() => handleOpenModal(assignment)}
            className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-yellow-500 to-amber-500 px-4 py-2 text-sm font-medium text-white shadow-lg transition-transform duration-200 hover:scale-105 hover:from-yellow-600 hover:to-amber-600 hover:shadow-xl focus:outline-none focus:ring-2 focus:ring-yellow-300"
            aria-label={`Edit response for ${assignment.title}`}
          >
            <svg
              className="h-4 w-4"
              fill="none"
      ...
      case 'verified':
        return (
          <button
            type="button"
            onClick={() => handleOpenModal(assignment)}
            className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-green-600 to-emerald-600 px-4 py-2 text-sm font-medium text-white shadow-lg transition-transform duration-200 hover:scale-105 hover:from-green-700 hover:to-emerald-700 hover:shadow-xl focus:outline-none focus:ring-2 focus:ring-green-300"
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-purple-50 to-indigo-100 p-4 sm:p-6">
      <div className="mx-auto max-w-5xl">
        <div className="mb-6">
          <h1 className="mb-2 text-2xl sm:text-3xl font-bold text-gray-800">Assignments</h1>
          <div className="h-1 w-28 rounded-full bg-gradient-to-r from-purple-500 to-indigo-600"></div>
        </div>

        {assignments.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16">
            <div className="mb-6 flex h-20 w-20 items-center justify-center rounded-full bg-gradient-to-br from-purple-100 to-indigo-200">
              <svg
            ...
            <h3 className="mb-2 text-lg sm:text-xl font-semibold text-gray-700">No Assignments Available</h3>
            <p className="max-w-md text-center text-gray-500">
              No assignments have been posted for this topic yet. Check back later for new
              assignments.
            </p>
          ...
          <div className="space-y-6">
            {assignments.map((assignment, index) => {
              ...
              return (
                <div
                  key={assignment._id}
                  className={`group transform rounded-2xl border border-white/20 bg-white/80 p-4 sm:p-6 shadow-lg backdrop-blur-sm transition-all duration-300 ease-out hover:-translate-y-1 hover:shadow-xl ${
                    overdue ? 'bg-red-50/30 ring-2 ring-red-200' : ''
                  }`}
                  style={{ animationDelay: `${index * 100}ms` }}
                >
                  <div className="mb-4 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                    <div className="flex-1">
                      ...
                      <div className="mb-3 flex flex-wrap items-center gap-3">
                        <div
                          className={`inline-flex items-center gap-2 rounded-full px-3 py-1 text-sm font-medium ${statusConfig.bg} ${statusConfig.text}`}
                        >
                          {statusConfig.icon}
                          <span>{statusConfig.label}</span>
                        </div>
