    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 p-4 sm:p-6">
      <div className="mx-auto max-w-4xl">
        ...
        <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h2 className="text-2xl sm:text-3xl font-bold text-gray-800">Live Sessions</h2>
          ...
          <div className="flex w-full flex-wrap gap-2 sm:w-auto">
            {(['all', 'live', 'scheduled', 'ended'] as const).map((f) => (
              <button
                key={f}
                onClick={() => setFilter(f)}
                className={`min-w-[84px] flex-1 rounded-xl px-3 py-1.5 text-sm font-medium transition ${
                  filter === f
                    ? 'bg-indigo-600 text-white'
                    : 'bg-white text-slate-700 border border-slate-200 hover:bg-slate-50'
                }`}
                ...
                aria-label={`${f[0].toUpperCase() + f.slice(1)} filter`}
              >
                {f[0].toUpperCase() + f.slice(1)}
              </button>
            ))}
        ...
        {loading ? (
          <div
            className="flex items-center justify-center py-20 text-slate-600"
            role="status"
            aria-live="polite"
          >
            Loading sessions…
          ...
          <div className="py-16 text-center">
            ...
            <h3 className="mb-2 text-lg sm:text-xl font-semibold text-gray-700">No sessions found</h3>
            <p className="max-w-md mx-auto text-gray-500">
              Your tutor hasn’t started any live sessions for this topic yet.
            </p>
          </div>
          ...
          <div className="space-y-4">
            {sessions.map((s) => (
              <div
                key={s._id}
                className="flex flex-col items-stretch gap-3 rounded-2xl border border-white/20 bg-white/80 p-4 sm:flex-row sm:items-center sm:justify-between sm:p-5 shadow-lg backdrop-blur-sm"
              >
                <div className="flex-1">
                  ...
                  <div className="mt-2 flex flex-wrap items-center gap-2 text-sm text-slate-500">
                    ...
                    <span className={pill(s.status)} aria-hidden>
                ...
                <div className="flex w-full gap-2 sm:w-auto sm:flex-none">
                  {s.status === 'live' ? (
                    <Link
                      to={`/student/live/${s._id}`}
                      className="w-full rounded-xl bg-indigo-600 px-4 py-2 text-center text-white shadow hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-300"
                    >
                      Join
                    </Link>
                  ) : (
                    <button
                      disabled
                      className="w-full rounded-xl bg-slate-200 px-4 py-2 text-slate-600 cursor-not-allowed"
                      title={s.status === 'scheduled' ? 'Not live yet' : 'Session ended'}
                      aria-disabled={true}
                    >
                      Join
