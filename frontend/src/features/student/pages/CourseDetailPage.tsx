    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 p-4">
        <div className="max-w-md rounded-2xl bg-white p-6 text-center shadow-xl">
          ...
          <h3 className="mb-2 text-lg font-bold text-gray-800">Error Loading Course</h3>
          <p className="mb-4 text-red-600">{error}</p>
          <button
            onClick={() => nav(-1)}
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 p-4">
        <div className="max-w-md rounded-2xl bg-white p-6 text-center shadow-xl">
          ...
          <h3 className="mb-2 text-lg font-bold text-gray-800">Course Not Found</h3>
          <p className="mb-4 text-gray-600">The course you're looking for doesn't exist.</p>
          <button
            onClick={() => nav(-1)}
            className="rounded-xl bg-blue-600 px-4 py-2 text-white transition-colors hover:bg-blue-700"
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
          ...
          <main className="col-span-1 space-y-6 lg:col-span-8">
            ...
            <div className="overflow-hidden rounded-3xl border border-gray-100 bg-white shadow-xl">
              ...
              <div className="bg-gradient-to-r from-blue-600 to-purple-600 p-6 sm:p-8 text-white">
                <h1 className="mb-2 text-2xl sm:text-4xl font-bold">{course.title}</h1>

                {/* NEW: Rating summary */}
                <div className="mt-3 flex flex-wrap items-center gap-3 text-blue-100">
                  <div className="flex items-center gap-2">
                    <Stars value={stats?.avg ?? 0} />
                    <span className="font-semibold text-white/90">
                      {(stats?.avg ?? 0).toFixed(1)}
                  ...
                  <span className="text-sm">{stats?.count ?? 0} review{(stats?.count ?? 0) === 1 ? '' : 's'}</span>
                </div>
              </div>
                ...
                {course.demoVideoUrl ? (
                  <div className="relative">
                    <video
                      ...
                      poster={course.thumbnail}
                    />
                    <div className="absolute left-3 top-3 rounded-full bg-black bg-opacity-50 px-3 py-1 text-sm text-white">
                      <Play className="mr-1 inline h-4 w-4" />
                      Demo Video
                    </div>
                  </div>
                  ...
                  <div className="relative">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center bg-black bg-opacity-40">
                      <div className="text-center text-white">
                        <Play className="mx-auto mb-2 h-12 w-12 opacity-80" />
                        <p className="text-lg font-semibold">Course Preview</p>
                      </div>
                    </div>
                  </div>
            ...
            <div className="rounded-2xl border border-gray-100 bg-white p-6 sm:p-8 shadow-lg">
              <h2 className="mb-4 flex items-center text-lg sm:text-2xl font-bold text-gray-800">
              ...
              <div className="flex flex-col items-start gap-4 sm:flex-row sm:items-start">
                {course.tutorProfilePic && (
                  <img
                    src={course.tutorProfilePic}
                    alt="Instructor"
                    className="h-16 w-16 rounded-full border-4 border-blue-100 object-cover shadow-lg"
                    loading="lazy"
                  />
                )}
                <div className="flex-1">
                  <h3 className="mb-2 text-lg font-bold text-gray-800">{course.tutorName}</h3>
                  <p className="mb-4 leading-relaxed text-gray-600">{course.tutorSummary}</p>
                  ...
                  <div className="grid gap-3 sm:grid-cols-2">
                    <div className="rounded-xl bg-blue-50 p-3">
                      <div className="mb-1 flex items-center gap-2">
                        ...
                        <span className="font-semibold text-gray-700 text-sm">Education</span>
                      </div>
                      <p className="text-sm text-gray-600">{course.tutorEducation || 'Not available'}</p>
                    </div>
                    <div className="rounded-xl bg-purple-50 p-3">
                      ...
                      <p className="text-sm text-gray-600">{course.tutorExperience || 'Not available'}</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="rounded-2xl border border-gray-100 bg-white p-6 sm:p-8 shadow-lg">
              ...
              <div className="prose prose-lg max-w-none">
                <p className="whitespace-pre-line leading-relaxed text-gray-700">{course.details}</p>
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-gray-100 bg-white p-6 sm:p-8 shadow-lg">
              <div className="mb-4 flex items-center justify-between">
                <h2 className="flex items-center text-lg sm:text-2xl font-bold text-gray-800">
                  <Star className="mr-3 h-5 w-5 text-yellow-500" fill="currentColor" />
                  Ratings & Reviews
                </h2>
                <div className="flex items-center gap-3">
                  <Stars value={stats?.avg ?? 0} />
                  <span className="text-sm font-semibold text-gray-800">{(stats?.avg ?? 0).toFixed(1)}</span>
                  <span className="text-xs text-gray-500">{stats?.count ?? 0} total</span>
                </div>
              </div>
              ...
              {!revLoading && reviews.length === 0 && <p className="text-gray-600">No reviews yet.</p>}

              <div className="space-y-3">
                {reviews.map((r) => (
                  <div key={r._id} className="rounded-xl border border-gray-100 p-3">
                    <div className="mb-2 flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        {r.studentAvatar ? (
                          <img
                        ...
                        <div>
                          <div className="font-medium text-gray-800">{r.studentName || 'Student'}</div>
                          <div className="text-xs text-gray-500">{formatDate(r.createdAt)}</div>
                        </div>
                      </div>
                      <Stars value={r.rating} />
                    </div>
                    {r.comment && <p className="mt-1 whitespace-pre-line text-gray-700">{r.comment}</p>}
                  </div>
                ))}
              </div>
              ...
              {hasMore && (
                <div className="mt-4 text-center">
                  <button
                    onClick={loadMore}
                    disabled={revLoading}
                    className="rounded-xl border border-gray-300 bg-white px-4 py-2 text-sm font-semibold text-gray-700 transition-colors hover:bg-gray-50 disabled:opacity-60"
                    type="button"
                  >
                    {revLoading ? 'Loading…' : 'Load more reviews'}
                  </button>
            ...
            <div className="text-center">
              <button
                className="inline-flex items-center space-x-2 rounded-xl bg-gray-600 px-4 py-2 font-semibold text-white shadow transition-all hover:bg-gray-700"
                onClick={() => nav(-1)}
                type="button"
              >
          ...
          <aside className="col-span-1 lg:col-span-4">
            <div className="sticky top-6 space-y-6">
              {/* Purchase Card */}
              <div className="rounded-2xl border border-gray-100 bg-white p-4 sm:p-6 shadow-xl">
                <div className="mb-4 text-center">
                  <div className="mb-2 text-2xl sm:text-4xl font-bold text-gray-800">₹{course.price}</div>
                ...
                <div className="mt-4 border-t border-gray-200 pt-4">
                  <h4 className="mb-3 text-center text-sm font-semibold text-gray-800">What's Included</h4>
                  <div className="space-y-2">
                    <div className="flex items-center justify-between rounded-lg border border-blue-100 bg-blue-50 p-2">
                      <div className="flex items-center gap-2">
                        <div className="rounded-full bg-blue-500 p-2">
                          <BookOpen className="h-4 w-4 text-white" />
                        </div>
                        <span className="font-medium text-blue-800 text-sm">Notes Available</span>
                      </div>
                      <div className="flex items-center text-sm text-green-600">
                        <Award className="mr-1 h-4 w-4" />
                        <span>Ready</span>
                    ...
                    <div className="flex items-center justify-between rounded-lg border border-red-100 bg-red-50 p-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-medium text-red-800 text-sm">Live Session</span>
                      </div>
                      <div className="flex items-center text-sm text-red-600">
                    ...
                    <div className="flex items-center justify-between rounded-lg border border-purple-100 bg-purple-50 p-2">
                      <div className="flex items-center gap-2">
                        <div className="rounded-full bg-purple-500 p-2">
                          <Play className="h-4 w-4 text-white" />
                        </div>
                        <span className="font-medium text-purple-800 text-sm">Recorded Classes</span>
                      </div>
                      <div className="flex items-center text-sm text-green-600">
                        <Clock className="mr-1 h-4 w-4" />
                        <span>24/7</span>
              ...
              {/* Course Modules */}
              <div className="rounded-2xl border border-gray-100 bg-white p-4 sm:p-6 shadow-lg">
                <h3 className="mb-3 flex items-center text-lg font-bold text-gray-800">
                  ...
                  {course.modules.map((m, idx) => (
                    <div key={m._id} className="overflow-hidden rounded-xl border border-gray-200">
                      ...
                      <button
                        ...
                        type="button"
                        aria-expanded={expandedModules.has(m._id)}
                      >
                        <div className="flex items-center gap-3">
                          <div className="flex h-8 w-8 items-center justify-center rounded-full bg-blue-100 text-sm font-bold text-blue-600">
                            {idx + 1}
                          </div>
                          <h4 className="flex-1 font-medium text-gray-800">{m.name}</h4>
                        </div>
                        <div className="flex items-center gap-2">
                          <span className="rounded-full bg-gray-100 px-2 py-1 text-xs text-gray-500">
                            {m.topics.length} topics
                            ...
                            {m.topics.map((t, topicIdx) => (
                              ...
                              >
                                <div className="flex items-start gap-3">
                                  ...
                                  <div className="flex-1">
                                    <h5 className="mb-1 font-semibold text-gray-800">{t.title}</h5>
