...
export const logoutStudentThunk = createAsyncThunk('student/logout', async (_, thunkAPI) => {
  try {
    await logoutStudent();
...
const studentAuthSlice = createSlice({
  ...
  extraReducers: (b) => {
    b.addCase(loginStudentThunk.pending, (s) => {
      s.loading = true;
      s.error = null;
    });
    ...
    b.addCase(googleLoginStudentThunk.pending, (s) => {
      s.loading = true;
      s.error = null;
    });
    ...
    b.addCase(googleLoginStudentThunk.rejected, (s, a) => {
      s.loading = false;
      s.error = a.payload as string;
    });
    ...
    b.addCase(logoutStudentThunk.rejected, (s, a) => {
      s.loading = false;
      s.error = a.payload as string;
    });
  },
